...
const _auth = {
	...
	commonEmailForm: function () {
		if (this.notEmail()) { // 이메일 형식에 맞지 않은 경우
			this.toggleErr(this.emailInput, this.emailError, "올바른 이메일 주소가 아닙니다.");
			return false
		}
		return true
	},//이메일 형식이 맞는지 검사 (~~~@~~.~~)
	
	isPasswordWritten: function (passwordInput = this.passwordInput, passwordError = this.passwordError) {
	...
	commonPasswordForm: function (){
			...
			// 비밀번호 검증 로직 
			if ((!isLongEnough || !hasLetters || !hasNumbers) && (this.passwordInput.value)) {
				// 조건을 충족하지 못하는 경우 에러 메시지 표시
				this.toggleErr(this.passwordInput, this.passwordError, "비밀번호는 영문, 숫자 조합 8자 이상 입력해 주세요.");
				return false
			}
			return true
...
export const auth = {
	...
	loginSubmissionChecker: function () {
		const{loginForm, emailInput, emailError, passwordInput, passwordError} = _auth
			...
			if (emailInput.value === "test@codeit.com" && passwordInput.value === "codeit101") {  //로그인 성공
				...
				if (_auth.isEmailWritten()) {
					if (_auth.commonEmailForm()) {
						if (emailInput.value !== "test@codeit.com") {  // 이메일 틀림
							_auth.toggleErr(emailInput, emailError, "이메일을 확인해 주세요.");
						}
					}
				}

				if(_auth.isPasswordWritten()){
					if (passwordInput.value !== "codeit101") {  // 비밀번호 틀림
						_auth.toggleErr(passwordInput, passwordError, "비밀번호를 확인해 주세요.");
					}
				}
			}
		});
	...
	passwordCheckFocusEvent: function () {
		const { passwordCheck: passwordInput, passwordCheckError: passwordError } = _auth;
		passwordInput.addEventListener("focusout", () => {  // focusout시 입력 형식 올바른지 검사
	...
	signupSubmissionChecker: function () {
		const {signupForm, emailInput, emailError, passwordInput, passwordError, passwordCheck, passwordCheckError} = _auth
		...
		signupForm.addEventListener("submit", function (event) {  //폼 제출 시 로그인 시도 
			...
			// 이메일 검사
			if(validity = _auth.isEmailWritten()){ // 이메일 작성여부 검사
				if(validity = _auth.commonEmailForm()){ // 이메일의 형식이 올바른지 검사
					if (validity = !(emailInput.value === "test@codeit.com")) {  // 이미 존재하는 아이디는 아닌지 검사
					} else {
						_auth.toggleErr(emailInput, emailError, "이미 사용 중인 이메일입니다.");
					}
				}
			...
			//비밀번호 검증 로직
			if(validity = _auth.isPasswordWritten()){ // 비밀번호 작성여부 검사
			...
			if(passwordCheck.value !== passwordInput.value){
				_auth.toggleErr(passwordCheck, passwordCheckError, "비밀번호가 일치하지 않아요.")
				validity = 0
			}
			isValid = validity && isValid;
	
			if (isValid){ // isValid가 1일 때, 즉 오차가 없을 시 -> 폼 제출 성공
				window.location.href = "folder.html";
			}
		});
	}
