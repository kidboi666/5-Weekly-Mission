...
export const getFolderData = async () => {
  try {
    const response = await fetch(`${API_PATH}/api/users/1/folders`);

    if (!response.ok) {
      throw new Error('유저의 폴더 정보 데이터 불러오기 실패');
    }
...
export const getLinkData = async () => {
  try {
    const response = await fetch(`${API_PATH}/api/users/1/links`);
...
export const getLinkItemData = async ({ folder_id }) => {
  try {
    const response = await fetch(`${API_PATH}/api/users/1/links?folderId=${folder_id}`);

    if (!response.ok) {
      throw new Error('데이터 불러오는데 실패');
    }
