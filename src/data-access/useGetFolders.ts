 ...
 * @returns {boolean} return.loading - 폴더 데이터를 가져오는 요청의 로딩 상태입니다.
 * @returns {any | null} return.error - 폴더 데이터를 가져오는 요청 중 발생한 오류입니다.
 * @returns {Array} return.data - 매핑되고 정렬된 폴더 데이터 배열입니다.
 * @returns {number} return.data[].id - 폴더의 고유 ID입니다.
 * @returns {string} return.data[].createdAt - 폴더 생성 일자입니다.
 * @returns {string} return.data[].name - 폴더의 이름입니다.
 * @returns {number} return.data[].userId - 폴더 소유자의 사용자 ID입니다.
 * @returns {number} return.data[].linkCount - 폴더에 포함된 링크의 개수입니다.
 *
 * @example
