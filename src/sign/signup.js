...
document
  ...
  .addEventListener("focusout", function () {
    ...
    if (
      !hasInput(passwordCheck, passwordCheck_error, "비밀번호를 확인해 주세요.")
    ) {
      return;
    }
...
document.getElementById("eye_toggle2").addEventListener("click", function () {
  ...
  const type =
    passwordCheck_input.getAttribute("type") === "password"
      ? "text"
      : "password";
...
async function checkEmailDuplicate(email) {
  ...
  if (response.ok) {
    ...
    return result.data.exists; // 중복 여부 확인
  } else {
    return { error: "사용 중인 이메일입니다." };
  }
}
...
async function submitUser() {
  ...
  if (isDuplicate && isDuplicate.error) {
    email_error.textContent = isDuplicate.error;
    return;
  }
  ...
  if (!validatePassword(passwordCheck)) {
    passwordCheck_error.textContent =
      "비밀번호는 영문, 숫자 조합 8자 이상 입력해 주세요.";
    return;
  }
  ...
  if (signUpResponse) {
    location.href = "../folder.html"; // 회원가입 성공 시 이동
  } else {
