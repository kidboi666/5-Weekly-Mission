...
import { checkEmail } from "/static/api/auth/signupApi.js";
...
const validateEmail = async function() {
    ...
    if(emailValue === '') {
        ...
        if(!eamilRegex.test(emailValue)) { // 올바른 이메일 형식이 아닐 때
            ...
            $email.classList.add("wrong");
            return;
        } else if($email.classList.contains("join")) {
            const resultCheckEmail = await checkEmail(emailValue);

            if(!resultCheckEmail.success) {
                if(resultCheckEmail.status === 409){
                    $emailError.textContent = resultCheckEmail.message;
                    $email.classList.add("wrong");
                    return;
                }else if(resultCheckEmail.status === 400){
                    $emailError.textContent = "올바른 이메일 주소가 아닙니다.";
                    $email.classList.add("wrong");
                    // alert(resultCheckEmail.message);
                    return;
                }
            }
        }
